...
HRESULT CALLBACK IOleInPlaceActiveObject_TranslateAccelerator_Proxy(
    ...
    LPMSG lpmsg)
{
    WARN("(%p): doesn't work remotely\n", lpmsg);
    return S_OK; /* like native, this call isn't remotable */
}
...
HRESULT __RPC_STUB IOleCache2_UpdateCache_Stub(
    ...
    LONG_PTR pReserved)
{
    FIXME("(%p, 0x%08x, 0x%08lx): check me\n", pDataObject, grfUpdf, pReserved);
    return IOleCache2_UpdateCache(This, pDataObject, grfUpdf, (LPVOID)pReserved);
}
