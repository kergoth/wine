...
#   define szNativeDesktop "/My Native Desktop"
    ...
    for (i=0; i < sizeof(aidsMyStuff)/sizeof(aidsMyStuff[0]); i++) {
        ...
        else
        {
            ...
            for (l=1; l >= 0; l--)
            {
                for (u=0; folder_mappings[i + 1].unix_location[u] != 0; u++)
                {
                    strcpy(szMyStuffTarget, szPersonalTarget);
                    if (_SHAppendToUnixPath(szMyStuffTarget, MAKEINTRESOURCEW(folder_mappings[i + 1].unix_location[u]), l) &&
            ...
            else
            {
                 ...
                 */
                strcpy(szMyStuffTarget, szPersonalTarget);
                if (!_SHAppendToUnixPath(szMyStuffTarget, MAKEINTRESOURCEW(folder_mappings[i + 1].ids), localize))
                {
