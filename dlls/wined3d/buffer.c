...
BYTE *buffer_get_sysmem(struct wined3d_buffer *This, struct wined3d_context *context)
{
    ...
    {
        char *data = GL_EXTCALL(glMapBufferRange(This->buffer_type_hint, 0, This->resource.size, GL_MAP_READ_BIT));
        memcpy(This->resource.heap_memory, data, This->resource.size);
...
static void buffer_direct_upload(struct wined3d_buffer *This, const struct wined3d_gl_info *gl_info, DWORD flags)
{
    ...
    if (cxgames_hacks.allow_glmapbuffer == WINED3D_MAPBUF_NEVER
            || (cxgames_hacks.allow_glmapbuffer == WINED3D_MAPBUF_STATIC
            && This->resource.usage & WINED3DUSAGE_DYNAMIC))
    {
        if (flags & WINED3D_BUFFER_DISCARD)
        {
            GL_EXTCALL(glBufferData(This->buffer_type_hint, This->resource.size,
        ...
        while (This->modified_areas)
        {
            ...
            GL_EXTCALL(glBufferSubData(This->buffer_type_hint, start, len,
                    (char *)This->resource.heap_memory + start));
        ...
        const struct wined3d_state *state)
{
    ...
    if (state)
    {
        ...
        if (!use_vs(state))
        {
            if (!gl_info->supported[ARB_VERTEX_ARRAY_BGRA] && !context->d3d_info->d3dcolor_swizzle)
...
HRESULT CDECL wined3d_buffer_map(struct wined3d_buffer *buffer, UINT offset, UINT size, BYTE **data, DWORD flags)
{
    ...
    if (wined3d_use_csmt(buffer->resource.device) && count == 1)
    {
        ...
        if (flags & WINED3D_MAP_DISCARD && !swvp)
        {
            ...
            {
                /* Even if our old user memory block is no longer in use,
                 * the GL VBO might be. Apple's GL drivers still want a
                 * glBufferData(NULL) call to avoid glitches. */
                wined3d_cs_emit_buffer_swap_mem(device->cs, buffer, NULL);
            }
        }
        ...
        const struct wined3d_sub_resource_data *data, void *parent, const struct wined3d_parent_ops *parent_ops)
{
    ...
    {
        TRACE("Using doublebuffer mode, GLES with no glMapBufferRange support.\n");
        buffer->flags |= WINED3D_BUFFER_DOUBLEBUFFER;
    }
