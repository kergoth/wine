...
static void wined3d_cs_exec_gl_texture_callback(struct wined3d_cs *cs, const void *data)
{
    ...
    struct wined3d_texture_gl *texture_gl = wined3d_texture_gl(op->texture);
    struct wined3d_texture_gl *depth_texture_gl = wined3d_texture_gl(op->depth_texture);
    ...
    wined3d_texture_load_location(&texture_gl->t, 0, context, WINED3D_LOCATION_TEXTURE_RGB);
    if (depth_texture_gl)
        wined3d_texture_load_location(&depth_texture_gl->t, 0, context, WINED3D_LOCATION_TEXTURE_RGB);

    op->callback(texture_gl->texture_rgb.name,
            depth_texture_gl ? depth_texture_gl->texture_rgb.name : 0,
    ...
    if (depth_texture_gl)
        wined3d_resource_release(&depth_texture_gl->t.resource);
    wined3d_resource_release(&texture_gl->t.resource);
}
