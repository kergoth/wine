...
void device_create_dummy_textures(struct wined3d_device *device, struct wined3d_context *context)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        if (gl_info->supported[EXT_TEXTURE3D])
        {
            ...
            GL_EXTCALL(glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8, 1, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, &color));
        ...
        struct wined3d_surface *dst_surface, const POINT *dst_point)
{
    ...
    dst_box.left = dst_point->x;
    dst_box.top = dst_point->y;
    dst_box.front = 0;
    dst_box.right = dst_point->x + update_w;
    dst_box.bottom = dst_point->y + update_h;
    dst_box.back = 1;
    if ((dst_surface->container->resource.format_flags & WINED3DFMT_FLAG_BLOCKS)
            && !wined3d_resource_check_block_align(&dst_surface->resource, &dst_box))
    {
