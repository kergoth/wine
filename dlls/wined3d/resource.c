        ...
        struct wined3d_context *context, DWORD location)
{
    ...
    if (src.buffer_object)
    {
        ...
        {
            char *data = GL_EXTCALL(glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, size, GL_MAP_READ_BIT));
...
HRESULT wined3d_sub_resource_map(struct wined3d_resource *resource,
        struct wined3d_map_desc *map_desc, const struct wined3d_box *box, DWORD flags)
{
    struct wined3d_device *device = resource->device;
    BYTE *base_memory;
...
HRESULT wined3d_sub_resource_unmap(struct wined3d_resource *resource)
{
    struct wined3d_device *device = resource->device;
    TRACE("resource %p.\n", resource);
