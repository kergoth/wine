...
void wined3d_swapchain_activate(struct wined3d_swapchain *swapchain, BOOL activate)
{
    ...
    else
    {
        ...
        old_style = GetWindowLongW(swapchain->device_window, GWL_STYLE);
        style = old_style | WS_MINIMIZEBOX | WS_SYSMENU;
        if (style != old_style)
        {
            SetWindowLongW(swapchain->device_window, GWL_STYLE, style);
            SetWindowPos(swapchain->device_window, NULL, 0, 0, 0, 0,
                    SWP_FRAMECHANGED | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER);
        }
        ShowWindow(swapchain->device_window, SW_MINIMIZE);
        if (style != old_style)
        {
            SetWindowLongW(swapchain->device_window, GWL_STYLE, old_style);
            SetWindowPos(swapchain->device_window, NULL, 0, 0, 0, 0,
                    SWP_FRAMECHANGED | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER);
        }
    }
