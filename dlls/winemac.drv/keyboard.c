...
void macdrv_edit_menu_command(const macdrv_event *event)
{
    ...
    if (mac_edit_menu == MAC_EDIT_MENU_BY_KEY)
    {
        ...
        BOOL alt_down, control_down, need_control = TRUE;
        WORD alt_scan, control_scan;
        DWORD flags;
        ...
        key_scan = macdrv_MapVirtualKeyEx(key_vkey, MAPVK_VK_TO_VSC, 0);

        alt_down = (GetAsyncKeyState(VK_MENU) & 0x8000) != 0;
        if (need_control)
            control_down = (GetAsyncKeyState(VK_CONTROL) & 0x8000) != 0;

        if (alt_down)
            alt_scan = macdrv_MapVirtualKeyEx(VK_LMENU, MAPVK_VK_TO_VSC, 0);
        if (need_control && !control_down)
            control_scan = macdrv_MapVirtualKeyEx(VK_LCONTROL, MAPVK_VK_TO_VSC, 0);

        if ((alt_down && !alt_scan) || (need_control && !control_down && !control_scan) || !key_scan)
        {
        ...
        if (alt_down)
        {
            /* Alt key is down but we don't want it down for the Control-C/V/X/...
            ...
            flags = KEYEVENTF_KEYUP;
            if (alt_scan & 0x100) flags |= KEYEVENTF_EXTENDEDKEY;
            macdrv_send_keyboard_input(hwnd, VK_LMENU, alt_scan & 0xff, flags,
