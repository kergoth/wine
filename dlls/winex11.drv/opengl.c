...
static EGLBoolean (*peglGetConfigs)(EGLDisplay dpy, EGLConfig *configs,
			 EGLint config_size, EGLint *num_config);
static EGLBoolean (*peglChooseConfig)(EGLDisplay dpy, const EGLint *attrib_list,
			   EGLConfig *configs, EGLint config_size,
			   EGLint *num_config);
static EGLBoolean (*peglGetConfigAttrib)(EGLDisplay dpy, EGLConfig config,
			      EGLint attribute, EGLint *value);

static EGLSurface (*peglCreateWindowSurface)(EGLDisplay dpy, EGLConfig config,
				  EGLNativeWindowType win,
				  const EGLint *attrib_list);
static EGLSurface (*peglCreatePbufferSurface)(EGLDisplay dpy, EGLConfig config,
				   const EGLint *attrib_list);
...
static EGLBoolean (*peglQuerySurface)(EGLDisplay dpy, EGLSurface surface,
			   EGLint attribute, EGLint *value);
...
static EGLSurface (*peglCreatePbufferFromClientBuffer)(
	      EGLDisplay dpy, EGLenum buftype, EGLClientBuffer buffer,
	      EGLConfig config, const EGLint *attrib_list);

static EGLBoolean (*peglSurfaceAttrib)(EGLDisplay dpy, EGLSurface surface,
			    EGLint attribute, EGLint value);
...
static EGLBoolean (*peglMakeCurrent)(EGLDisplay dpy, EGLSurface draw,
...
static EGLBoolean (*peglQueryContext)(EGLDisplay dpy, EGLContext ctx,
			   EGLint attribute, EGLint *value);
...
static EGLBoolean (*peglCopyBuffers)(EGLDisplay dpy, EGLSurface surface,
...
static GLXContext create_glxcontext(Display *display, struct wgl_context *context, GLXContext shareList)
{
    ...
    if(context->gl3_context)
    {
        if(context->numAttribs)
            ctx = pglXCreateContextAttribsARB(gdi_display, context->fmt->config.fbc, shareList, GL_TRUE, context->attribList);
        else
...
static void free_gl_drawable( struct gl_drawable *gl )
{
    ...
    case DC_GL_PIXMAP_WIN:
        pglXDestroyGLXPixmap( gdi_display, gl->drawable.d );
...
static BOOL create_glx_drawable( HWND hwnd, struct gl_drawable *gl )
{
    ...
    else
    {
        ...
        if (gl->pixmap)
        {
            gl->drawable.d = pglXCreateGLXPixmap( gdi_display, gl->visual, gl->pixmap );
...
static void wglFinish(void)
{
    ...
    if ((gl = get_gl_drawable( WindowFromDC( ctx->hdc ), 0 )))
    {
        ...
        case DC_GL_CHILD_WIN:  escape.gl_drawable = gl->drawable.d; break;
        default: break;
        }
        sync_context(ctx);
        release_gl_drawable( gl );
...
static BOOL X11DRV_wglBindTexImageARB( struct wgl_pbuffer *object, int iBuffer )
{
    ...
    if (use_render_texture_emulation) {
        ...
        */
        ...
        tmp_context = pglXCreateNewContext(gdi_display, object->fmt->config.fbc, object->fmt->render_type, prev_context, True);

        opengl_funcs.gl.p_glGetIntegerv(object->texture_bind_target, &prev_binded_texture);

        /* Switch to our pbuffer */
        pglXMakeCurrent(gdi_display, object->pbuffer.drawable, tmp_context);
...
static int describe_egl_context(struct wgl_context *ctx)
{
    int tmp;
    int ctx_vis_id;
    TRACE(" Context %p have (vis:%p):\n", ctx, ctx->vis);
    peglGetConfigAttrib(display, ctx->fmt->config.c, EGL_CONFIG_ID, &tmp);
    TRACE(" - CONFIG_ID 0x%x\n", tmp);
    peglGetConfigAttrib(display, ctx->fmt->config.c, EGL_NATIVE_VISUAL_ID, &tmp);
    TRACE(" - NATIVE_VISUAL_ID 0x%x\n", tmp);
    ctx_vis_id = tmp;
    return ctx_vis_id;
}
...
static BOOL egldrv_wglMakeCurrent(HDC hdc, struct wgl_context *ctx)
{
    ...
    if ((gl = get_gl_drawable(WindowFromDC(hdc), hdc)))
    {
        ...
        if (TRACE_ON(wgl))
        {
            describe_egl_context(ctx);
            TRACE("hdc %p drawable %lx EGL drawable %p fmt %p ctx %p\n", hdc, gl->drawable.d, gl->drawable.s, gl->format, ctx->ctx.egl);
        }
