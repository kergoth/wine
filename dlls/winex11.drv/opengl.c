...
static BOOL set_surface_format( struct gl_drawable *gl, const struct wgl_pixel_format *format )
{
    ...
    TRACE( "created GL drawable %lx for surface %p format %x\n", gl->drawable, gl->surface, format->fmt_id );
...
/**********************************************************************
 *              gl_fullscreen_cursor_window
 */
HWND gl_fullscreen_cursor_window(Window window)
{
    struct wgl_surface *surface;
    HWND ret = NULL;
    ...
    switch (xev->type)
    {
        case ClientMessage:
                ...
                (Atom)xev->xclient.data.l[0] == x11drv_atom(WM_TAKE_FOCUS))
            {
                ...
                if (found)
                {
                    ...
                    XRaiseWindow(display, xev->xclient.window);
                ...
                else
                {
                    if (fullscreen)
                    {
                        ...
                        XRaiseWindow(display, fullscreen);
                        XSetInputFocus(display, fullscreen, RevertToParent, (Time)xev->xclient.data.l[1]);
                        return TRUE;
                    }

                    /* Allow normal processing to proceed to set win32 focus. */
                }
            }
            break;
        case ConfigureNotify:
            if (xev->xconfigure.window == root_window)
            {
                ...
                LIST_FOR_EACH_ENTRY(surface, &surface_list, struct wgl_surface, entry)
                {
                    if (surface->gl->type == DC_GL_FULLSCREEN_WIN)
                    {
                        TRACE("root window changed; moving full-screen surface/window %p/%lx to (%d,%d) %dx%d\n",
                              surface, surface->gl->drawable, changes.x, changes.y, changes.width, changes.height);
                        XReconfigureWMWindow(display, surface->gl->drawable, DefaultScreen(display),
        ...
        case FocusIn:
        {
            ...
            else
            {
                if (fullscreen)
                {
                    ...
                    XRaiseWindow(display, fullscreen);
                    XSetInputFocus(display, fullscreen, RevertToParent, time);
                }
        ...
        case GravityNotify:
            if (is_gl_fullscreen_window(xev->xgravity.window))
            {
                ...
                TRACE("root window changed; moving full-screen window %lx to (%d,%d) %dx%d\n",
        ...
        case MapNotify:
            if (!managed_mode)
            {
                ...
                if (surface && proxy == GetDesktopWindow())
                {
                    Time time;
                    ...
                    XRaiseWindow(display, xev->xmap.window);
                    XSetInputFocus(display, xev->xmap.window, RevertToParent, time);
                }
            }
            break;
    }
